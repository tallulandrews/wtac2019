...
if (@ARGV < 3) {die "Please supply: a GTF of gene annotations, a directory of Kallisto Output and a prefix for output\n";}
...
foreach my $file (@files) {
	...
	while (<$ifh>) {
		...
		if (exists($transcript2gene{$trans})) { 
			$gene = $transcript2gene{$trans}; #Aggregate by gene
		}
